...
class CacheCoherence(base_handler.BaseHandler):
  ...
  valid_types = [
    "cachecoherence",    # Description to Components
  ]
  # this function is for randomizing which letters are on or 
  #off in each proceesor
  ...
  def validate_answer(self,pro,l,letter):
		a=''
		if l == " loads":
  ...
  def data_for_question(self,question_type):
		...
		pro=self.generator.randint(0,2)
		l=self.generator.choice([' loads',' stores'])
		letter=self.generator.choice(['A ','B ','C '])
		answer =''
		if letter =='A ':
			answer = self.validate_answer(pro,l,A)
		if letter =='B ':
			answer = self.validate_answer(pro,l,B)
		if letter =='C ':
			answer = self.validate_answer(pro,l,C)
		description = ('Processor '+str(pro+1)
		+l + ' '+letter)
		return {"description":description,'A4i':A4i,'B4i': B4i,'C4i': C4i,'A4':A4,'B4':B4,'C4':C4,'A1' : A[0][0],'A2' : A[1][0],'A3' :A[2][0],'A1i':A[0][1],'A2i':A[1][1],'A3i':A[2][1],'SharedA':A[0][2],'SharedA2':A[1][2],'SharedA3':A[2][2],'B1' : B[0][0] ,'B2' : B[1][0],'B3' : B[2][0],'B1i' :B[0][1],'B2i' :B[1][1],'B3i' :B[2][1],'SharedB': B[0][2] ,'SharedB2':B[1][2],'SharedB3':B[2][2],'C1':C[0][0],'C2':C[1][0],'C3':C[2][0],'C1i':C[0][1],'C2i':C[1][1],'C3i':C[2][1],'SharedC':C[0][2],'SharedC2':C[1][2],'SharedC3':C[2][2],'answer':answer
		}
  def score_student_answer(self,question_type,question_data,student_answer):
      wanted = self.get_description_string(question_data)
      if wanted == student_answer or wanted.lower() == student_answer:
        return (100.0, wanted)
      else:
        return (0.0,wanted)
